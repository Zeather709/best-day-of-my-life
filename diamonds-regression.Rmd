...
The `carat` variable is > 1 order of magnitude less than that of the `table` variable (and so close to 1 OOM smaller than `depth`), so we will go ahead and standardize all 3. However this should occur after the data set has been split into the training and testing data sets.  
...
XGBoost uses gradient boosted decision trees and is a really robust model which performs very well on a broad variety of applications.  It is also not as sensitive to issues that affect the performance of other models such as multicolinearity, or data normalization/standardization.
...
This takes the decision tree model a step further and uses many decision trees with different weights to make better predictions than if you were using a single decision tree. 
```{r fores, warning = FALSE, message = FALSE}
...
Now we will predict the log of the price of diamonds in the test data set useing each model we trained to determine which model performs best on data it has not seen.  
